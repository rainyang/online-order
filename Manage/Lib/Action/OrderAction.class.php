...
class OrderAction extends CommonAction {
	...
	public function index()
	{
		...
		if($_GET['status'] == "0")
		{
			$list = $Order->field('on_member.account,on_member.email,on_member.telephone,on_member.address,on_billhead.id,on_billhead.restaurant_id,on_billhead.bh_no,on_billhead.bh_type,on_billhead.bh_status,on_billhead.bh_create_time')->where(array('on_billhead.bh_status' => 0))->join('on_member ON on_billhead.bh_customer_id = on_member.id')->order('id desc')->select();
		...
		{
			$list = $Order->field('on_member.account,on_member.email,on_member.telephone,on_member.address,on_billhead.id,on_billhead.restaurant_id,on_billhead.bh_no,on_billhead.bh_type,on_billhead.bh_status,on_billhead.bh_create_time')->where(array('on_billhead.bh_status' => 6))->join('on_member ON on_billhead.bh_customer_id = on_member.id')->order('id desc')->select();
			foreach($list AS $key => $row)
			{
				$list[$key]['bh_create_time'] = date('d/m/Y',$list[$key]['bh_create_time']);
				if($list[$key]['bh_type'] == 3)
		...
		{
			$list = $Order->field('on_member.account,on_member.email,on_member.telephone,on_member.address,on_billhead.id,on_billhead.restaurant_id,on_billhead.bh_no,on_billhead.bh_type,on_billhead.bh_status,on_billhead.bh_create_time')->where(array('bh_status' => 3))->join('on_member ON on_billhead.bh_customer_id = on_member.id')->select();
			foreach($list AS $key => $row)
			{
				$list[$key]['bh_create_time'] = date('d/m/Y',$list[$key]['bh_create_time']);
				if($list[$key]['bh_type'] == 3)
