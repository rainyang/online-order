...
$.jQchart={
	version  : '0.02-beta-2.1',
	updateMemo : 'v0.02-beta-2.1 are going to support "use : gradient"',
		...
		jQuery.extend( $.fn.jQchart,{
			...
			init :	function (op){ 
				...
				this.op=jQuery.extend({
					...
					use 		: op.config.use	|| null,   // 'gradient:#canvasMyID'
				...
				if(
					this.op.id==this.op.use_api_box &&
					this.op.use_api=='gradient'
				)
					this.useGradient(this.op.id,this.op);
			...
			useGradient : function (id,op){
			
				var grdnt=$('<div></div>')
					.attr('id'   ,'jQchart-bg-'+id)
					.attr('class','jQchart-bg')
					.css('width',$("#"+id).css('width'))
					.css('height',$("#"+id).css('height'))
				
				var grdnt=$("#"+id).wrap(grdnt)
				if(typeof op.config.gradient=='undefined')op.config.gradient={};
				var cfg=op.config.gradient;
				$("#jQchart-bg-"+id).gradient({
					from      : cfg.from  ||'222222', 
					to        : cfg.to    ||'687478',
					direction : cfg.direction ||'horizontal',  //'vertical'|'horizontal'
					position  : cfg.position  ||'top left',
