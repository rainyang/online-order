...
/*				if(op.ckbox) {//2011年10月31日16:28:03 去掉自动选择子分类功能
		...
		_checkParent:function(){
			...
			$(">div>.ckbox", parent).removeClass("unchecked").removeClass("checked").removeClass(rClass).addClass(aClass).find("input");
			//2011年10月31日15:50:43 袁吉 选择下级菜单项后！让上级菜单也选中 begin
			var ckbox = $(">div>.ckbox", parent);
			var cked = ckbox.hasClass("checked")?true:ckbox.hasClass("indeterminate")?true:false;
			$("input", ckbox).attr("checked", cked);
			//2011年10月31日15:50:43 袁吉 选择下级菜单项后！让上级菜单也选中 end
			parent._checkParent();
		}
	});
})(jQuery);