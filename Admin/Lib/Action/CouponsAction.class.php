...
class CouponsAction extends Action{
	public function index(){
		...
		$Coupons = M('Coupons');
		$result = $Coupons->where(array('res_id' => $res_id))->select();
	...
	public function edit(){
		...
		if($_POST['coupons_id']){
            $result = M('Coupons')->where(array('id'=>$_POST['coupons_id']))->save($data);
			if($data['type'] == 3){
				M('CouponsItem')->where(array('coupons_id'=>$_POST['coupons_id']))->delete();
				foreach($_POST['item_list'] as $k=>$v){
					$add_coupons_item = M('CouponsItem')->add(array('coupons_id'=>$_POST['coupons_id'],'item_id'=>$k,'num'=>$_POST['item_num'][$k]));
				}	
			}
			$this->redirect('/Coupons/index');
